...
return {
    ...
    -- =====================================================================
    -- Dock — a second bar at the bottom edge, generated by scripts/dock.py
    --
    -- On by default, which it was not: windows now have a minimize button, and
    -- a minimized window has to be visible SOMEWHERE or it is simply gone. The
    -- dock's taskbar lists every window — GTK, Qt and XWayland alike, because
    -- it speaks the foreign-toplevel protocol rather than a toolkit's — so it
    -- is where a minimized window comes back from. Turn it off in the settings
    -- app if you would rather work without one; minimize then still works, and
    -- SUPER+SHIFT+M shows what is on the minimized workspace.
    -- =====================================================================
    dock = {
        enabled   = true,
        ...
        height    = 52,
        margin    = 8,
        -- Waybar has no true autohide. This drops the reserved space so
        -- windows tile underneath and the dock floats over them; it does not
        -- slide away when unused.
        autohide  = false,
        -- Desktop entry ids, without ".desktop".
        pinned    = { "brave-origin", "kitty", "nemo", "code" },
    },
