...
declare -gA MSG=(
    ...
    [warn_update]="The system update did not finish cleanly: %s"
    ...
    [fail_hyprland_old]="Hyprland %s is too old. The configuration here is Lua, which needs the %s series."
    ...
    [fail_flatpak]="Flatpak %s could not be installed: %s"
    ...
    [step_colour_includes]="Wiring the generated colours into tmux and git"
    ...
    [err_low_space]="Only %s MB free on /. This install needs about %s GB. Stopping here rather than getting most of the way and failing: use %s, or give the disk more room."
    [err_low_space_flatpak]="Only %s MB free on /var/lib/flatpak, which is its own filesystem here. The Flatpaks need about 5 GB. Use --no-flatpak, or enlarge it."
    [warn_flatpak_no_space]="Only %s MB free for Flatpaks, about 5 GB is needed — skipping all of them instead of failing on each"
    [fail_hyprland_new]="Hyprland %s is newer than the %s series this desktop is written for. The version pin in packages/copr-desktop.txt did not hold."
    [ask_language]="Language / Sprache?"
    [ask_pick]="Pick a number"
    [ask_default]="default"
    [ask_other]="something else"
    [ask_invalid]="%s is not one of the accepted values"
    [ask_kb_layout]="Keyboard layout?"
    [ask_kb_layout_free]="Layout code (as in localectl list-x11-keymap-layouts)"
    [ask_kb_variant]="Keyboard variant? (- for none)"
    [ask_kb_variant_free]="Variant (- for none)"
    [ask_timezone]="Timezone?"
    [ask_timezone_free]="Timezone (Area/City)"
    [ask_hostname]="Computer name?"
    [ask_flavour]="Colour palette?"
    [ask_accent]="Accent colour?"
    [sec_setup]="Basic settings"
    [info_setup_skipped]="unattended run: keeping the current keyboard, timezone and hostname"
    [step_keymap]="Setting the keyboard to %s %s (session, login screen and console)"
    [warn_keymap]="The keyboard layout could not be set"
    [step_timezone]="Setting the timezone to %s"
    [warn_timezone]="The timezone could not be set"
    [step_hostname]="Setting the computer name to %s"
    [warn_hostname]="The computer name could not be set"
    [info_hostname_kept]="computer name stays %s"
    [ok_setup]="keyboard %s, timezone %s, palette %s/%s"
    [step_settings_keyboard]="Writing keyboard %s %s into settings.lua"
    [warn_settings_keyboard]="The keyboard layout could not be written to settings.lua"
    [warn_copr_exclude]="COPR %s could not be restricted to its own packages"
)
